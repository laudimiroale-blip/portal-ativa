...
async function inicializarChecklist(codigoOperacao: string, produto: string, estadoCivil?: string) {
  ...
  const checklistPorProduto: Record<string, { nome: string; categoria: string; opcional?: boolean }[]> = {
    ...
    "Rural Equity": [
      ...
      { nome: "Matrícula atualizada do imóvel", categoria: "Imóvel Rural" },
    ...
    "Imóvel em Construção": [
      ...
      { nome: "Extratos bancários PF — 3 meses", categoria: "Renda" },
      { nome: "Contracheques — 3 meses (CLT)", categoria: "Renda", opcional: true },
      { nome: "Matrícula atualizada do imóvel", categoria: "Obra" },
...
export const operacoesRouter = router({
  ...
  criar: protectedProcedure
    .input(z.object({
      ...
      categoriaGarantia: z.enum(["Residencial","Comercial","Rural","Veicular","Construção"]).optional(),
      garantiaQuitada: z.enum(["Sim — totalmente quitada","Não — possui financiamento ativo","Parcialmente financiada"]).optional(),
      dividaAtual: z.string().optional(),
      finalidadePrincipal: z.enum(["Capital de giro","Quitação de dívidas","Expansão empresarial","Investimento operacional","Reforma","Construção","Término de obra","Compra de equipamentos","Reorganização financeira","Liquidez","Investimento rural","Outros"]).optional(),
      origemRenda: z.enum(["Assalariado (CLT)","Empresário / Sócio","Profissional liberal","Produtor rural","Aposentado / Pensionista","Comissionado","Misto","Outros"]).optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await createOperacao({
        ...
        categoriaGarantia: input.categoriaGarantia,
        garantiaQuitada: input.garantiaQuitada,
        dividaAtual: input.dividaAtual ? parseMoney(input.dividaAtual) : undefined,
        finalidadePrincipal: input.finalidadePrincipal,
        origemRenda: input.origemRenda,
  ...
  atualizar: protectedProcedure
    .input(z.object({
      ...
      resumoInteligente: z.string().optional(),
