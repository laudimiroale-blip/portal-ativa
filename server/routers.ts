    ...
    criar: protectedProcedure
      .input(
        z.object({
          ...
          responsavelOperacionalId: z.number(),
          prioridade: z.enum(["Baixa", "Normal", "Alta", "Urgente"]).default("Normal"),
          statusRascunho: z.boolean().default(false),
          observacoesEstrategicas: z.string().optional(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        await createOperacao({
          ...
          observacoesEstrategicas: input.observacoesEstrategicas,
          statusMacro: "Pré-cadastro",
          statusValidacaoIa: "Não analisado",
        });

        // Inicializar checklist de documentos por produto
  ...
  documentos: router({
    ...
    upload: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Atualizar status macro para Documentação parcial
    ...
    atualizarEstado: adminProcedure
      .input(
        z.object({
          ...
          estado: z.enum(["Pendente", "Enviado", "Em Análise", "Aprovado", "Reprovado", "Ilegível", "Vencido", "Reenviar"]),
  ...
  ia: router({
    ...
    analisarDocumental: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const analiseResult = await createAnaliseIa({
          operacaoId: input.operacaoId,
          camada: "documental",
...
Observações Estratégicas: ${op.observacoesEstrategicas ?? "Nenhuma"}
...
async function inicializarChecklist(codigoOperacao: string, produto: string) {
  // Buscar a operação pelo código para obter o ID
